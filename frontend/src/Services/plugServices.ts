...
class plugServices {
  ...
  async isConnected(): Promise<boolean> {
    try {
      ...
      console.error("check plug connection error : ", error);
      return false;
    }
  }
  ...
  async initIcpLedgerActor(): Promise<boolean> {
    try {
      this.icpLedger = await window.ic?.plug?.createActor({
        canisterId: icpLedgerCanister,
        interfaceFactory: icpFactory,
      });
      return true;
    } catch (e) {
      console.error("init ICP ledger Actor through plug error", e);
      return false;
    }
  }
  ...
  async login(): Promise<boolean> {
    try {
      console.log(
        "plug login - whitelist - host - icpLedgerCanister : ",
        whitelist,
        host,
        icpLedgerCanister
      );
      await window.ic?.plug?.requestConnect({ whitelist, host });
      this.principalId = window.ic.plug.principalId;
      this.icpLedger = await window.ic?.plug?.createActor({
        canisterId: icpLedgerCanister,
        interfaceFactory: icpFactory,
      });
  ...
  async icp_approve(amount: bigint): Promise<boolean> {
    try {
      ...
      console.error("plug approve ICP error : ", error);
      return false;
    }
  }
...
const plugServiceInstance = new plugServices();
export default plugServiceInstance;
